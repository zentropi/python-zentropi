...
class Agent(BaseAgent):
    ...
    async def start(
        ...
        logger.info(f"Agent {self.name} is starting.")
        self._running = True
    ...
    async def _ensure_connection(self):
        ...
        try:
            await self._connection.connect(endpoint=self._endpoint, token=self._token)
            self._connected = True
        except PermissionError:
            logger.fatal(
                f"Login failed when connecting to {self._endpoint} with token {self._token!r}"
            )
        ...
        try:
            ...
            self.spawn("frame-receive-loop", self._frame_receive_loop(), single=True)
            await self.filter_frames()
            if self._joined_spaces:
                await self.join(self._joined_spaces)
            elif self._join_all_spaces:
                await self.join("*")
            self.spawn("frame-send-loop", self._frame_send_loop(), single=True)
        except Exception as e:
            logger.exception(e)
            self.stop()
    ...
    async def request(self, _name: str, timeout: int, **_data):
        ...
        try:
            return await asyncio.wait_for(
                self._wait_for_response(frame), timeout=timeout
            )
        except Exception as e:
